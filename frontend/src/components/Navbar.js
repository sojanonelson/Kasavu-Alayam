      ...
      <nav className={`${navBar ? (isScrolled ? 'bg-white/95 backdrop-blur-xl border-b border-gray-200/20' : 'bg-white/10 backdrop-blur-2xl') : (isScrolled ? 'bg-white/95 backdrop-blur-xl border-b border-gray-200/50' : 'bg-white')} transition-all duration-500`}>
        ...
        <div className="hidden lg:flex lg:px-32 items-center justify-between px-8 py-4">
          ...
          <div className="flex items-center space-x-8">
            ...
            {localStorage.getItem("user") && (
              <button onClick={() => handleLogout()} className={`${navBar ? (isScrolled ? "text-black" : "text-white") : "text-black"} transition-colors duration-500 hover:scale-110 transform`}>
                <LogOut size={22} />
              </button>
            )}
        ...
        <div className="lg:hidden">
          <div className="flex items-center justify-between px-3 py-2">
            {/* Hamburger Menu */}
            <button onClick={() => setIsOpen(!isOpen)} className={`${navBar ? (isScrolled ? 'text-black' : 'text-white') : 'text-black'} transition-all duration-500 p-1`}>
            ...
            <div className="flex items-center space-x-2">
              ...
              {localStorage.getItem("user") && (
                <button onClick={() => handleLogout()} className={`${navBar ? (isScrolled ? 'text-black' : 'text-white') : 'text-black'} transition-all duration-500 p-1`}>
                  <LogOut size={18} />
                </button>
              )}
            </div>
