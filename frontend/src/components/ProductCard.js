...
const ProductCard = ({ product, isListView, wishlist, toggleWishlist, toggleCart }) => {
  ...
  const colorDots = ['White', 'Black', 'Pink', 'Red']
    ...
    //             <h3 className="text-md font-medium">{product.id}</h3>
    //           </div>
  ...
  <Link className="flex-1" to={`/details/${product.id}`}>
    <motion.button
      whileHover={{ scale: 1.05 }}
      whileTap={{ scale: 0.95 }}
      className="w-full bg-blue-600 hover:bg-blue-700 text-white py-1.5 text-xs rounded-md flex items-center justify-center transition-all duration-300"
      aria-label="Buy now"
    >
      Buy Now
    </motion.button>
